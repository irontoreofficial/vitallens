...
export class VitalLensPanel implements vscode.WebviewViewProvider {
  ...
  private buildPayload(result: AnalysisResult) {
    ...
    const imageIssuesCount = result.issues.filter(i => i.category === IssueCategory.Image).length;
    const bundleIssuesCount = result.issues.filter(i => i.category === IssueCategory.Bundle).length;
    const redirectIssuesCount = result.issues.filter(i => i.category === IssueCategory.Redirect).length;
    const seoIssuesCount = result.issues.filter(i => i.category === IssueCategory.SEO).length;
    ...
    const inpIssuesCount = result.issues.filter(i => i.ruleId === 'JS-SEO-001' || i.category === IssueCategory.Bundle).length;
    let inpStatus = 'Good';
    if (inpIssuesCount > 3) {
      inpStatus = 'Poor';
