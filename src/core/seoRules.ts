...
export interface SeoRule {
  id: string;
  title: string;
  message?: string; // Explains Why + Instead of + Use this
  recommendation: string; // Used in CodeLens title
  badCode: string;
  goodCode: string;
  why: string;
  severity: IssueSeverity;
  category: IssueCategory;
  languages: string[]; // extensions: e.g. ['html', 'vue', 'tsx', 'jsx', 'css', 'js', 'ts']
  regex: RegExp;
  // Optional generator for QuickFix
  getQuickFix?: (document: vscode.TextDocument, match: RegExpExecArray, matchIndex: number) => vscode.WorkspaceEdit | null;
}
...
export function getRulesForFile(fileName: string): SeoRule[] {
  const ext = fileName.split('.').pop()?.toLowerCase() || '';
  return SEO_RULES.filter((rule) => rule.languages.includes(ext));
}

export function formatSeoMessage(rule: SeoRule): string {
